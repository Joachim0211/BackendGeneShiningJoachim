...
const writeGeneData = (data, file) => {
  const uploadFolder = path.resolve("public", "uploads");
  const pathToFile = `${uploadFolder}/${file.filename}.json`
  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(
      ...
      (error) => {
        if (error) {
          reject("Writing gene data to disk failed :(");
        } else {
          resolve(pathToFile);
        }
      }
...
module.exports = { readGeneData, writeGeneData };
